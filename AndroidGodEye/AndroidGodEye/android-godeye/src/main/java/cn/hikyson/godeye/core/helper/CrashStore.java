...
public class CrashStore {
    public static Observable<List<CrashInfo>> observeCrashAndCache(final Context context,final String storeDirName) {//FDS fix method arg could be final
    ...
    private static final FilenameFilter mCrashFilenameF = (dir, filename) -> filename.endsWith(SUFFIX);//FDS fix long var

    private static synchronized void storeCrash(final Context context,final List<CrashInfo> crashInfos,final String storeDirName) {//FDS fix method arg could be final
        List<File> crashFiles = null;
        try {
            crashFiles = Arrays.asList(Objects.requireNonNull(makeSureCrashDir(context, storeDirName).listFiles(mCrashFilenameF)));
        } catch (Throwable e) {
            L.e(e);
        }
        if (crashFiles != null && crashFiles.size() >= sMaxStoreCount) {
            ...
            for (int i = 0; i < (crashFiles.size() + 1 - sMaxStoreCount); i++) {
                ...
                } catch (FileNotFoundException e) {//FDS fix avoid generic exception
                    L.e(e);
                }
            }
        }
    ...
    private static void deleteIfExists(final File file) throws Exception {//FDS fix method arg could be final
        if (file.exists() && !file.delete()) {
            throw new Exception("deleteIfExists failed");
        }
    }

    private static synchronized List<CrashInfo> restoreCrash(final Context context,final String storeDirName) {//FDS fix method arg could be final
    ...
    private static String getStoreFileName(final long crashTime) {//FDS fix method arg could be final
        return FORMATTER.format(new Date(crashTime)) + SUFFIX;
    }

    private static File getStoreFile(final Context context,final String fileName,final String storeDirName) throws IOException {//FDS fix method arg could be final
    ...
    private static File makeSureCrashDir(final Context context,final String storeDirName) throws IOException {//FDS fix method arg
        File crashDir;
