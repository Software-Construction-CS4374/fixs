...
class ViewCanaryInternal {
    ...
    void stop(final Application application) {
        if (callbacks == null) {
            return;
        }
    ...
    @VisibleForTesting
    Runnable inspectInner(final WeakReference<Activity> activity,final ViewCanary viewCanary,final ViewCanaryConfig config,final Map<Activity, List<List<ViewIdWithSize>>> recentLayoutListRecords) {
        return () -> {
            try {
...
//            L.d("ViewCanary interrupt because %s's layout is similar to someone in records, cost %sms.", activity.getClass().getSimpleName(), (System.currentTimeMillis() - startTime));
    ...
    private static boolean allNotSimilarByMeasureDistanceLayoutEigenvalueWithRecords(List<List<ViewIdWithSize>> records, List<ViewIdWithSize> layoutEigenvalue) {
        ...
        WeightedLevenshtein l1 = new WeightedLevenshtein(new ViewWithSizeSubstitutionInterface() {
            ...
            @Override
            public double deletionCost(final ViewIdWithSize c) {
                return c.sizeInScreenPercent;
            }

            @Override
            public double insertionCost(final ViewIdWithSize c) {
                return c.sizeInScreenPercent;
            }
        });
    ...
    private Rect calculateOverDraw(final Rect r1,final Rect r2) {
