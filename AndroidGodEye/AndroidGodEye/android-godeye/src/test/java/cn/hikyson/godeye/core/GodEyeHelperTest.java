...
public class GodEyeHelperTest {
    ...
    public void onPageLoadedWhenNotCreated() {
        ...
        try {
            ...
            testObserver.assertValueAt(0, new Predicate<PageLifecycleEventInfo>() {
                @Override
                public boolean test(final PageLifecycleEventInfo object) throws Exception {
                    return activity.hashCode() == object.pageInfo.pageHashCode
                            && ActivityLifecycleEvent.ON_LOAD.equals(object.currentEvent.lifecycleEvent);
                }
            }).assertValueAt(1, new Predicate<PageLifecycleEventInfo>() {
                @Override
                public boolean test(final PageLifecycleEventInfo object) throws Exception {
                    return fragment.hashCode() == object.pageInfo.pageHashCode
                            && FragmentLifecycleEvent.ON_LOAD.equals(object.currentEvent.lifecycleEvent);
                }
            });
        } catch (UninstallException e) {
            fail();
    ...
    public void onPageLoadedSuccessForActivity() {
        try {
            GodEye.instance().uninstall();
            GodEye.instance().install(GodEyeConfig.noneConfigBuilder().withPageloadConfig(new PageloadConfig()).build());
            ActivityController<Test1Activity> activityCon = Robolectric.buildActivity(Test1Activity.class).create().start().resume();
            Test1Activity activity = activityCon.get();
            ...
            GodEyeHelper.onPageLoaded(activity);
            activityCon.pause().stop().destroy();
            Shadows.shadowOf(ThreadUtil.obtainHandler("godeye-pageload").getLooper()).getScheduler().advanceToNextPostedRunnable();
            ...
            for (int i = 0; i < 8; i++) {
                ...
                testObserver.assertValueAt(i, new Predicate<PageLifecycleEventInfo>() {
                    @Override
                    public boolean test(final PageLifecycleEventInfo object) throws Exception {//FDS fix short var in lines 157,113,230,291,436,119,376,481
                        return testPageEvents.get(finalI).pageHashCode == object.pageInfo.pageHashCode
                                && testPageEvents.get(finalI).allEventSize == object.allEvents.size()
                                && testPageEvents.get(finalI).lifecycleEvent.equals(object.currentEvent.lifecycleEvent);
                    }
                });
            }
        } catch (UninstallException e) {
    ...
    public void onPageLoadedSuccessWithFragment() {
        try {
            GodEye.instance().uninstall();
            GodEye.instance().install(GodEyeConfig.noneConfigBuilder().withPageloadConfig(new PageloadConfig()).build());
            ActivityController<Test2Activity> activityCon = Robolectric.buildActivity(Test2Activity.class).create().start().resume();
            Test2Activity activity = activityCon.get();
            GodEyeHelper.onPageLoaded(activity);
            TestObserver testObserver = GodEye.instance().<Pageload, PageLifecycleEventInfo>moduleObservable(GodEye.ModuleName.PAGELOAD).test();
    ...
    public void onFragmentPageVisibilityChangeSuccess() {
        try {
            ...
            ActivityController<Test2NotV4FragmentActivity> activityCon = Robolectric.buildActivity(Test2NotV4FragmentActivity.class).create().start().resume();
            activity = activityCon.get();
    ...
    public void onAppStartEndSuccess() {
        try {
            ...
            testObserver.assertValue(new Predicate<StartupInfo>() {
                @Override
                public boolean test(final StartupInfo object) throws Exception {
                    return startupInfo.startupType.equals(object.startupType) && startupInfo.startupTime == object.startupTime;
                }
            });
        } catch (Throwable e) {
            fail();
    ...
    public void onNetworkEndSuccess() {
        try {
            ...
            testObserver.assertValue(new Predicate<NetworkInfo>() {
                @Override
                public boolean test(final NetworkInfo object) throws Exception {//FDS fix method arg could be final lines 481,230,291,119,376,436,157
                    return networkInfo.isSuccessful == object.isSuccessful && networkInfo.message.equals(object.message);
                }
            });
        } catch (Throwable e) {
            fail();
