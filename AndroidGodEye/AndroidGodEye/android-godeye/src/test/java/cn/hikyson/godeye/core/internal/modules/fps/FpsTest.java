    ...
    public void work() {
        try {
            ...
            testObserver.assertValueCount(3).assertValueAt(0, new Predicate<FpsInfo>() {
                @Override
                public boolean test(final FpsInfo info) throws Exception {//FDS fix method arg would be final lines 51,57 and 63
                    Log4Test.d(info);
                    return info.currentFps == 7;
                }
            }).assertValueAt(1, new Predicate<FpsInfo>() {
                @Override
                public boolean test(final FpsInfo info) throws Exception {
                    Log4Test.d(info);
