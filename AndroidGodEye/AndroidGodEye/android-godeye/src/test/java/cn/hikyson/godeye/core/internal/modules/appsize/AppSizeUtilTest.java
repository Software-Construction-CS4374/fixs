...
public class AppSizeUtilTest {
	AppSizeUtilTest(){}//FDS fix at least one Constructor
    ...
    public void getAppSizeLowerO() {
        ...
        AppSizeUtil.getAppSize(ApplicationProvider.getApplicationContext(), new AppSizeUtil.OnGetSizeListener() {
            @Override
            public void onGetSize(final AppSizeInfo ctAppSizeInfo) {//FDS fix method arg could be final
                countDownLatch.countDown();
                appSizeInfo[0] = ctAppSizeInfo;
            }

            @Override
            public void onError(final Throwable throwable) {//FDS fix method arg could be final and short var
                countDownLatch.countDown();
            }
        });
        try {
