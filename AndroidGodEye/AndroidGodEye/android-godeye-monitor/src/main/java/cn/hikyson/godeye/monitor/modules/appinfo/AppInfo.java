...
public class AppInfo implements Serializable {
	public AppInfo() {//FDS fix at least one constructor
		
	}
    private static cn.hikyson.godeye.core.monitor.AppInfoConext sAppInfoConext;

    @Keep
    public static void injectAppInfoConext(final cn.hikyson.godeye.core.monitor.AppInfoConext appInfoConext) {//FDS fix method arg could be final
        ...
        public static AppInfo create() {
            ...
            synchronized (AppInfo.class) {
                if (TextUtils.isEmpty(sAppName)) {
                    PackageManager pManager = context.getPackageManager();//FDS fix short var
                    sAppName = context.getApplicationInfo().loadLabel(pManager).toString();
                }
                if (sInternalLabels == null) {
                    ...
                    if (PermissionChecker.checkSelfPermission(context, Manifest.permission.ACCESS_COARSE_LOCATION) == PermissionChecker.PERMISSION_GRANTED
                        ...
                        double[] location = easyLocationMod.getLatLong();//FDS fix short var
                        String lat = String.valueOf(location[0]);
                        String lon = String.valueOf(location[1]);
